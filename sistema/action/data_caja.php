		...
		$usuario = $_SESSION['idUser'];
			...
			$total_pagina = ceil($total_registro / $por_pagina);
				...
				while ($data = mysqli_fetch_assoc($query)){
					...
					$detalleTabla .= '<tr>
						                <td>'.$data['fecha'].'</td>
						                <td class="">'.number_format($data['inicio'],2)." ".$moned.'</td>
										
						                <td class="">'.number_format($data['ventas'],2)." ".$moned.'</td>
						               
						                <td class="">'.number_format($data['egresos'],2)." ".$moned.'</td>
						                <td class="">'.number_format($data['total_efectivo'],2)." ".$moned.'</td>
