				...
				if ($result > 0) {
					while ($data = mysqli_fetch_array($query)) {
						...
						if ($data["totalventa"] == 0) {
							$estatus = '<span class="pagada">Devolución</span>';
							$totalventa= $moned.' '.number_format($data["totalventa"],2);
						}
							...
							<td>
								...
								if ($data['totalventa'] != 0) {
									$detalleTabla.='<div>
										...
										<a href class="btn_view textcenter" title="Ver ticket" onclick="event.preventDefault(); verTicket('.$data['codcliente'].','.$data['noventa'].');"><i class="class="far fa-edit"></i>Ticket</a>
										<a href class="btn_view textcenter" title="Ver ticket" onclick="event.preventDefault(); verFactura('.$data['codcliente'].','.$data['noventa'].');"><i class="class="far fa-edit"></i>Factura</a>
								...
								if ($data["status"] == 1 || $data['status'] == 3 || $data['status'] == 4)
								 {
									$detalleTabla.= '<div class="div_factura">
										<a href="#" class="btn_anular anular_factura" title="Eliminar venta" onclick="event.preventDefault();
