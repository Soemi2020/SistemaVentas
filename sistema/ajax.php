	   ...
	   //Extraer datos del detalle_temp
	   	if ($_POST['action'] == 'serchForDetalle'){
			...
			if (empty($_POST['user'])) 
			{
				...
				$descuento = ($total_total*(16/100));
				$total_con_desc = $total_total;
				...
				$detalleTotales = '<tr>
					               ...
					               <tr>
						               <td colspan="5" class="textright">IVA incluido 16% ' .$moned.'</td>
						               <td class="textright">'.number_format($descuento,2).'</td>
						               <td></td>
					               </tr>
					               <tr>
						               <td colspan="5" class="textright">TOTAL ' .$moned.'</td>
						               <td class="textright">'.number_format($total_con_desc,2).'</td>
						               <td></td>
					               </tr>';
...
VALUES('$proveedor','$codigo','$producto','$costo','$precio','$usuario_id','$imgProducto')");
 	...
 	// Agregar New factura proveedor
	   ...
	   //Devolucion
		 if ($_POST['action'] == 'devolucion'){
		 	...
		 	if (empty($_POST['noVenta_dev']) || empty($_POST['codProducto_dev']) || empty($_POST['cantProducto_dev'])) {
				...
				if ($cantidad == $info_det_venta['cantidad']) {
					...
					// Verificar si la venta ya no tiene detalles asociados
					$query_check_details = mysqli_query($conection, "SELECT COUNT(*) as total FROM detalleventa WHERE noventa = $venta");
					$row_check_details = mysqli_fetch_assoc($query_check_details);
					$total_details = $row_check_details['total'];
			
					if ($total_details == 0) {
						// Si no hay detalles asociados a la venta, marcar la venta como devuelta o eliminarla
						// Por ejemplo, para marcarla como devuelta:
						//$query_update_sale = mysqli_query($conection, "UPDATE venta SET devuelta = 1 WHERE noventa = $venta");
						
						// O para eliminarla:
						$query_delete_sale = mysqli_query($conection, "DELETE FROM venta WHERE noventa = $venta");
					}
		...
		if ($_POST['action'] == 'devolucion_compra'){
			//print_r($_POST);
			if (empty($_POST['noCompra_dev']) || empty($_POST['codProducto_dev']) || empty($_POST['cantProducto_dev'])) {
				   $code = '1';
					 $msg = "Todos los campos son abligatorios.";
			}else{
			   $venta 		= $_POST['noCompra_dev'];
			   $codigo 	= $_POST['codProducto_dev'];
			   $cantidad 	= $_POST['cantProducto_dev'];
			   $user 		= $_SESSION['idUser'];
			   $result 	= 0;
			   ...
			   $query_det_venta = mysqli_query($conection,"SELECT * FROM entradas WHERE nocompra = $venta AND codproducto = $codproducto ");
			   $result_det_venta = mysqli_num_rows($query_det_venta);
				   if ($result_det_venta > 0) {
					   $info_det_venta = mysqli_fetch_assoc($query_det_venta);
				   }
			   ...
			   if ($cantidad > $info_det_venta['cantidad']){
				   ...
				   $cant_vendido = $info_det_venta['cantidad'];
				   $precio_vendido = $info_det_venta['precio'];
				   $totalventa_detalle = $cantidad * $precio_vendido;
				   $nueva_cantidad_det = $cant_vendido - $cantidad;

			   $query_venta = mysqli_query($conection,"SELECT * FROM compras WHERE nocompra = $venta");
			   $result_venta = mysqli_num_rows($query_venta);
				   if ($result_venta > 0) {
					   $info_venta = mysqli_fetch_assoc($query_venta);
					   $totalventa = $info_venta['totalcompra'];
				   }
				   $nuevo_total = $totalventa - $totalventa_detalle;
				   $nueva_cantidad = $existencia - $cantidad;

			   $query_upd_venta = mysqli_query($conection,"UPDATE compras SET totalcompra = $nuevo_total WHERE nocompra = $venta");

			   $query_upd_producto = mysqli_query($conection,"UPDATE producto SET existencia = $nueva_cantidad WHERE codproducto = $codproducto");

			   if ($cantidad == $info_det_venta['cantidad']) {
				   $query_upd_det_venta = mysqli_query($conection,"DELETE FROM entradas WHERE nocompra = $venta AND codproducto = $codproducto");
		   
				   // Verificar si la venta ya no tiene detalles asociados
				   $query_check_details = mysqli_query($conection, "SELECT COUNT(*) as total FROM entradas WHERE nocompra = $venta");
				   $row_check_details = mysqli_fetch_assoc($query_check_details);
				   $total_details = $row_check_details['total'];
				   ...
				   if ($total_details == 0) {
					   ...
					   // O para eliminarla:
					   $query_delete_sale = mysqli_query($conection, "DELETE FROM compras WHERE nocompra = $venta");
				   }
					   

			   
			   }else{
				   $query_upd_det_venta = mysqli_query($conection,"UPDATE entradas SET cantidad = $nueva_cantidad_det WHERE nocompra = $venta AND codproducto = $codproducto");
			   }
