<?php
    ...
    if(isset($_POST['nombre_proveedor'])) {
        ...
        $query_proveedor = mysqli_query($conection, "SELECT * FROM proveedor WHERE proveedor = '$nombre_proveedor'");
        $data_proveedor = mysqli_fetch_assoc($query_proveedor);
        echo json_encode($data_proveedor);
        exit; // Termina el script aquí para evitar la salida HTML
    }
?>
                            ...
                            <td class="textcenter">
                                <div class="wd50">
                                    <div id="acciones_venta">
										<a href="#" class="btn_ok textcenter" id="btn_anular_compra" style="display: <?php echo ($result_caja = 0) ? 'inline-block' : 'none'; ?>;" onclick="event.preventDefault(); anularCompra();"><i class="fas fa-ban"></i> Anular</a>
        ...
        $(document).ready(function(){
            ...
            $('#nom_proveedor').autocomplete({
                ...
                select: function(event, ui) {
                    var nombre_proveedor = ui.item.value;
                    $.ajax({
                        url: 'tu_script.php', // Cambia 'tu_script.php' por la ruta correcta a tu script PHP
                        type: 'POST',
                        ...
                        success:function(data){
                            ...
                            $('#dir_proveedor').val(data.dir_proveedor);
                            $('#con_proveedor').val(data.con_proveedor);
                        }
                    });
                }
            });
